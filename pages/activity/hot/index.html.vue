<template>
    <div class="Hot-box flex flex-v flex-v-c f">
        <bee-page>
            ...
            <bee-search>
                <!-- tab切换 -->
                <bee-tab :current="1"></bee-tab>
            </bee-search>
            <div class="line mb-16"></div>
            <div class="bee-content-width" style="margin: 0 auto">
                <!-- banner展示 -->
                <img class="img-ban mb-24" src="@/assets/images/hot/hot_banner.png" alt="" />
                <!-- 类别 -->
                <client-only>
                    <collapse-fiflter
                        :list="categoryList"
                ...
                <!-- 商品列表展示 -->
                <div v-if="jPList.length" class="flex flex-w">
                    <div
                    ...
                    >
                        <bee-product-item :key="index" :item-data="item">
...
export default {
    ...
    props: {},
    async asyncData({ $api }) {
        const res = await $api.product.getBoutiqueProductApi({ type: 'JP' });
        return {
            jPList: res
        };
    },
    ...
    methods: {
        ...
        // 获取商品
        async getList() {
            const res = await this.$api.product.getBoutiqueProductApi({ type: 'JP', cat1Id: this.cat1Id });
            this.jPList = res || [];
        }
    }
};
